...
print("Searching Scanning materials...")
...
print("Data CLEARLY ERRONEOUS MATERIALS:")
print("-" * 80)
erroneous_count = 0
erroneous_list = []
for mat in sorted(ERRONEOUS_MATERIALS):
    ...
    if count > 0:
        ...
        print(f"  FAIL {mat:35s}: {count:6,} occurrences")
...
if len(questionable_products) > 0:
    print("WARNING QUESTIONABLE MATERIALS (might be intentional):")
    print("-" * 80)
...
print("Data COMBINED (Erroneous + Questionable):")
print("-" * 80)
...
if len(erroneous_products) / len(df) < 0.001:  # Less than 0.1%
    print("OK The erroneous materials affect a very small portion of the dataset")
    ...
    print("WARNING The erroneous materials affect a small portion of the dataset")
    print("  (<1%). Consider fixing rather than removing.")
else:
    print("ERROR The erroneous materials affect a significant portion of the dataset")

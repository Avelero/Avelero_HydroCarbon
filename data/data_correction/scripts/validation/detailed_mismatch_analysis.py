...
if mismatches:
    ...
    for m in sorted(mismatches, key=lambda x: x['count'], reverse=True):
        print(f"  ERROR {m['category']:30s}: Expected '{m['expected_parent']}', Got '{m['actual_parent']}' ({m['count']:,} products)")
...
if orphans:
    report_lines.append("### Orphaned/Unknown Categories\n")
    report_lines.append("| Category | Parent Category | Product Count |")
    report_lines.append("|----------|----------------|---------------|")
    for cat, parents in orphans:
        print(f"  WARNING Orphan: {cat:30s} - Parents: {parents}")
...
if gender_issues:
    report_lines.append("### Potentially Unusual Gender Assignments\n")
    ...
    for issue in sorted(gender_issues, key=lambda x: x['percentage'], reverse=True):
        print(f"  WARNING {issue['category']:30s}: {issue['count']:,} Male products out of {issue['total']:,} ({issue['percentage']:.2f}%)")
...
if naming_issues['duplicate_categories']:
    report_lines.append("### Overlapping/Duplicate Category Names\n")
...
for anom_type, count in weight_anomalies.items():
    if count > 0:
        ...
        if anom_type == 'extreme_low':
            ...
            print(f"  ERROR {count:,} products with weight < 10 grams")
        elif anom_type == 'extreme_high':
            desc = "> 50 kg (impossibly heavy)"
            print(f"  ERROR {count:,} products with weight > 50 kg")
        elif anom_type == 'zero_or_negative':
            desc = "Zero or negative weight"
            print(f"  ERROR {count:,} products with zero/negative weight")
        elif anom_type == 'missing':
            desc = "Missing weight data"
            print(f"  WARNING {count:,} products missing weight")
...
for anom_type, count in distance_anomalies.items():
    if count > 0:
        ...
        if anom_type == 'extreme_low':
            ...
            print(f"  ERROR {count:,} products with distance < 1 km")
        elif anom_type == 'extreme_high':
            desc = "> 40,000 km (> Earth's circumference)"
            print(f"  ERROR {count:,} products with distance > 40,000 km")
        elif anom_type == 'zero_or_negative':
            desc = "Zero or negative distance"
            print(f"  ERROR {count:,} products with zero/negative distance")
        elif anom_type == 'missing':
            desc = "Missing distance data"
            print(f"  WARNING {count:,} products missing distance")
...
# Save report
print(f"\nSaving Saving detailed report to: {REPORT_FILE}")

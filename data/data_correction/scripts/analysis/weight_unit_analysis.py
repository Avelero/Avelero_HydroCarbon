...
for range_name, count in magnitude_ranges.items():
    ...
    if '< 1g' in range_name or '< 0.001 kg' in range_name:
        interp = "ERROR Too light - likely unit error"
    elif '1-10g' in range_name or '0.001-0.01 kg' in range_name:
        interp = "WARNING Very suspicious - unlikely for clothing"
    elif '10-100g' in range_name:
        interp = "WARNING Possible - accessories or unit error"
    elif '100-1000g' in range_name or '0.1-1 kg' in range_name:
        interp = "PASS Normal for clothing"
    elif '1-5 kg' in range_name:
        interp = "PASS Normal for heavy items"
    elif range_name == 'heavy (5-10 kg)':
        interp = "WARNING Unusual but possible"
    else:
        interp = "ERROR Impossible for single garment"
...
if len(very_low) > 0:
    ...
    if pct_corrected > 80:
        report.append(f"\nCRITICAL **STRONG EVIDENCE**: {pct_corrected:.1f}% correction rate suggests these ARE in grams!\n")
        print(f"  CRITICAL STRONG EVIDENCE: {pct_corrected:.1f}% would be corrected!")
    elif pct_corrected > 50:
        report.append(f"\nWARNING **MODERATE EVIDENCE**: {pct_corrected:.1f}% correction rate suggests possible unit mixing\n")
        print(f"  WARNING MODERATE EVIDENCE: {pct_corrected:.1f}% would be corrected")
    else:
        report.append(f"\nPASS **UNLIKELY**: Only {pct_corrected:.1f}% would be corrected - probably not unit error\n")
        print(f"  PASS Only {pct_corrected:.1f}% would be corrected - unlikely to be unit error")
    print()
...
if len(very_high) > 0:
    ...
    if pct_corrected_high > 80:
        report.append(f"\nCRITICAL **STRONG EVIDENCE**: {pct_corrected_high:.1f}% correction rate - these ARE in grams!\n")
        print(f"  CRITICAL STRONG EVIDENCE: {pct_corrected_high:.1f}% would be corrected!")
    elif pct_corrected_high > 50:
        report.append(f"\nWARNING **MODERATE EVIDENCE**: {pct_corrected_high:.1f}% correction rate suggests possible unit mixing\n")
        print(f"  WARNING MODERATE EVIDENCE: {pct_corrected_high:.1f}% would be corrected")
    else:
        report.append(f"\nPASS **UNLIKELY**: Only {pct_corrected_high:.1f}% would be corrected\n")
        print(f"  PASS Only {pct_corrected_high:.1f}% would be corrected")
    print()
...
if len(weight_sample_filtered) > 100:
    ...
    if shapiro_p < 0.05:
        print(f"  FAIL Data is NOT normally distributed (p < 0.05)")
        report.append(f"**Normality Test (Shapiro-Wilk)**:\n")
        report.append(f"- Statistic: {shapiro_stat:.6f}\n")
        report.append(f"- P-value: {shapiro_p:.6e}\n")
        report.append(f"- **Result**: FAIL Data is NOT normally distributed (suggests multiple distributions mixed)\n\n")
    else:
        print(f"  OK Data appears normally distributed")
        report.append(f"**Normality Test**: OK Data appears normally distributed\n\n")
    print()
...
plt.close()
print(f"OK Saved visualization: weight_distribution_analysis.png\n")
...
if len(issues_found) > 0:
    report.append("### CRITICAL UNIT CONSISTENCY ISSUES DETECTED\n")
    report.append("\n**Evidence Summary**:\n")
    ...
    report.append("1. PASS **Fix low weights**: Multiply values < 0.01 kg by 1000\n")
    report.append("2. PASS **Fix high weights**: Divide values > 10 kg by 1000\n")
    report.append("3. PASS **Verify**: Check corrected values fall into expected ranges\n")
    report.append("4. PASS **Document**: Update data dictionary to confirm all weights in KG\n")
else:
    report.append("### PASS UNIT CONSISTENCY APPEARS ACCEPTABLE\n")
...
# Save report
print("Saving Saving detailed research report...")
with open(REPORT_FILE, 'w') as f:
    f.write('\n'.join(report))
print(f"OK Saved: {REPORT_FILE}\n")

print("=" * 80)
print("OK WEIGHT UNIT ANALYSIS COMPLETE")
print("=" * 80)

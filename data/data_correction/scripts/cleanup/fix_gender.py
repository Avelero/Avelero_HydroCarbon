...
# Create backup
print(f"Summary Creating backup: {BACKUP_CSV}")
...
print("Searching Removing corrupted header rows...")
before_count = len(df)
...
# Fix gender labels
print("Processing Fixing gender labels...")
...
print(f"OK Unique gender values after fix: {list(unique_genders)}")

if set(unique_genders) == {'Male', 'Female'}:
    print("OK SUCCESS: Only Male and Female remain")
elif set(unique_genders) == {'Female', 'Male'}:
    print("OK SUCCESS: Only Male and Female remain")
else:
    print(f"WARNING WARNING: Unexpected gender values found: {unique_genders}")

print()

# Save fixed data
print(f"Saving Saving fixed data to: {OUTPUT_CSV}")
df.to_csv(OUTPUT_CSV, index=False)
print(f"OK Saved {len(df):,} rows")

print()
print("=" * 80)
print("OK GENDER FIX COMPLETE")
print("=" * 80)
print()

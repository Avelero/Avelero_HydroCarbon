...
print(f"✓ Removed {removed_count_step1:,} products")
...
if len(orphan_dresses) > 0:
    ...
    print(f"✓ Reclassified {len(orphan_dresses)} product(s)")
...
if len(orphan_maxi_parent) > 0:
    ...
    print(f"✓ Fixed {len(orphan_maxi_parent)} product(s)")
...
print(f"\n✓ Standardized {sum(1 for k, v in category_mappings.items() if (df['category'] == k).sum() > 0)} category names\n")
...
print(f"✓ Updated materials in {materials_updated:,} products")
print(f"✓ Removed {removed_count_step4:,} products with impossible materials")
print(f"  Remaining: {len(df):,} products\n")
...
print(f"✓ Removed {removed_count_step5:,} outlier products")
print(f"  Remaining: {len(df):,} products\n")
